                ...
                switch (_c.label) {
                    case 0:
                        contractAddresses = contractAddresses.map(function (address) {
                            return tokens_1.removeEthFromContractAddress(address);
                        });
                        if (this._fiatPriceCache) {
                            now = Date.now();
                            if (deep_clone_1.deepClone(this._fiatPriceCache.timestamp) >
                    ...
                    case 1:
                        _c.trys.push([1, 4, , 5]);
                        coinGeckoResponse = {};
                        return [4 /*yield*/, node_fetch_1.default("https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=" + contractAddresses.join() + "&vs_currencies=usd")];
                    ...
                    case 3:
                        ...
                        for (_i = 0, _a = Object.entries(response); _i < _a.length; _i++) {
                            ...
                            for (i = 0; i < contractAddresses.length; i++) {
                                mappedKey = get_address_1.getAddress(key);
                                // @ts-ignore
